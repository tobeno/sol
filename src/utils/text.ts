export function filterLines(str: string, cb: (line: string) => boolean = (line: string) => !!line): string {
...
export function rfilterLines(str: string, cb: (line: string) => boolean): string {
...
export function replaceLines(str: string, pattern: string | RegExp, replacer: any) {
    return lines(str).map((line) => line.replace(pattern, replacer)).join('\n') + '\n';
}
